...
각각 모든 것을 할 수는 있지만, 언어의 특성을 따라서 골라서 쓰게 된다.
...
**SSH 에이전트 시작 및 키 추가**: 다음 명령어를 입력합니다.
...
#### 예제 16: 중첩 `for` 반복문을 사용한 2차원 배열 출력
```javascript
const matrix = [
    [1, 2, 3],
    [4, 5, 6],
...
#### 예제 19: `for...in` 반복문을 사용한 배열 인덱스 순회
```javascript
...
```
실제로 사용되는 모듈은 node.js모듈처럼 종류도 훨씬 많고 버전도 다양하기 때문에 이걸 제어하기 위해 npm 패키지제어시스템을 사용하는 것이다.(이하 npm, package.json)
...
//--> throw : 에러가 나면 컴퓨터 다운될 때까지 집착하지않고 집어던지고 다음 일을 하겠다는 뜻.
//--> 하지만 안써도 에러가 나면 catch로 보내버리는 기능이 JS에 들어있다.
//--> Error 는 class에 해당하는 객체임. const error = New Error("..."); return  error; 

} catch (error) {
    console.error(error.message); // "임의의 에러 발생"
//--> 주로 에러가 나면 해야 할 일. 또다른 try문이 들어있을 수도 있다.

} finally {
    console.log(" 이용해주셔서 감사합니다. 같은 에러가 나던 안나던 끝에선 보여줘야하는 것");
}
```
...
//--> 거의 쓰지않음.
//--> 변수자리에 꼭 문자열로!
- `alert(message)`: 경고 메시지 창을 표시합니다.(경고박스. 확인버튼만 있음.)
- `confirm(message)`: 확인 대화 상자를 표시하고, 사용자의 확인 여부를 불리언 값으로 반환합니다.(단순한 안내창, 취소 버튼이 있음. 확인버튼= true , 취소버튼= false반환)
- `prompt(message, default)`: 입력 대화 상자(input)를 표시하고, 사용자가 입력한 문자열을 반환합니다.(input에  default가 써져있는게 뜬다.)
...
#### Document 객체
//--> window.document 라고 콘솔에 명령하면 HTML을 보여줌.
//--> const document = window.document 이런 코드가 브라우저에 존재함.
//--> Dom 이랑 html 비슷하지만 같은 것은 아니다. 
//--> html은 class(설계도), Dom이 new객체같은 인스턴스임.
...
<body>
  <div id="myElement">This is a div with ID "myElement".</div>
...
ES6 모듈 시스템에서는 `export`와 `export default`를 사용하여 값을 내보낼 수 있으며, `import` 키워드를 사용하여 여러 가지 방법으로 모듈을 가져올 수 있습니다.
...
**1. 명명된 내보내기(`export`) 가져오기**

```javascript
// app.js
import { greeting, toUpperCase, toLowerCase } from './stringUtils.js';
...
console.log(stringUtils.greeting); // Hello, World!
console.log(stringUtils.toUpperCase('hello')); // HELLO
console.log(mathUtils.PI); // 3.14
console.log(mathUtils.add(2, 3)); // 5
console.log(mathUtils.subtract(5, 2)); // 3
console.log(mathUtils.default(2, 3)); // 6 (multiply 함수)
```

**4. 기본 내보내기와 명명된 내보내기 동시에 가져오기**
...
eventEmitter.on('processData', (data) => {
  console.log(`Handler 1: Received data - ${data}`);
});

// 두 번째 이벤트 핸들러
eventEmitter.on('processData', (data) => {
  console.log(`Handler 2: Logging data length - ${data.length}`);
});

// 비동기 이벤트 핸들러
eventEmitter.on('processData', (data) => {
  setTimeout(() => {
    console.log(`Handler 3: Async processing of data - ${data}`);
  }, 2000);
});

// 이벤트 발생
eventEmitter.emit('processData', 'Sample data for event processing');
```
...
##### 프로젝트 구조
#### 프로젝트 구조 해설
Next.js v14 이상에서는 `app` 디렉터리를 사용하여 라우트를 정의합니다. 다음은 각 디렉터리와 파일의 역할에 대한 설명입니다.
...
- **README.md**: 프로젝트에 대한 설명과 사용 방법을 포함하는 파일입니다. 일반적으로 프로젝트의 목적, 설치 방법, 사용법 등이 기술됩니다.
- **app/**: Next.js v14 이상에서 사용하는 주요 디렉터리로, 페이지와 레이아웃을 정의합니다.
  - **favicon.ico**: 브라우저 탭에 표시되는 아이콘 파일입니다.
  - **globals.css**: 프로젝트 전역에서 사용되는 CSS 파일로, 모든 페이지에 공통적으로 적용되는 스타일을 정의합니다.
  - **layout.js**: 모든 페이지에 공통적으로 적용되는 레이아웃을 정의하는 파일입니다. 일반적으로 헤더, 풋터와 같은 공통 요소를 포함합니다.
  - **page.js**: 기본 라우트(`/`)에 대한 컴포넌트를 정의하는 파일입니다. 이 파일의 내용은 기본 페이지로 렌더링됩니다.
  - **page.module.css**: 기본 라우트에 대한 스타일을 정의하는 CSS 모듈입니다. 컴포넌트별로 스타일을 캡슐화하여 글로벌 네임스페이스 충돌을 방지합니다.
- **jsconfig.json**: JavaScript 프로젝트 설정 파일로, 경로 별칭(alias) 및 기타 컴파일러 옵션을 설정합니다. TypeScript를 사용하는 경우 `tsconfig.json`을 사용합니다.
- **next.config.mjs**: Next.js의 설정 파일입니다. 이 파일을 통해 사용자 정의 설정을 적용할 수 있습니다. 예를 들어, 빌드 설정, 환경 변수 설정 등을 정의할 수 있습니다.
- **package-lock.json**: 프로젝트의 의존성 트리를 기록하는 파일로, 정확한 버전의 패키지를 설치하기 위해 사용됩니다. npm이 이 파일을 자동으로 생성하고 업데이트합니다.
- **package.json**: 프로젝트의 메타데이터와 의존성을 정의하는 파일입니다. 프로젝트의 이름, 버전, 스크립트, 의존성 등이 포함됩니다.
- **public/**: 정적 파일을 저장하는 디렉터리입니다. 이 디렉터리에 있는 파일들은 `/` 경로를 통해 접근할 수 있습니다.
...
##### 함수형 컴포넌트 작성 예제 및 상위 컴포넌트에서 props 주입하기
...
##### 상위 컴포넌트에서 props 주입하기

이제 이 두 컴포넌트를 상위 컴포넌트에서 사용하고 props를 전달하는 예제를 보겠습니다.
...
function App() {
  return (
    <div>
      <Welcome name="John" />
      <Greeting name="Jane" timeOfDay="morning" />
    </div>
  );
}
...
#### `<title>`
문서의 제목을 정의합니다. 브라우저의 제목 표시줄이나 탭에 표시됩니다.
...
#### `<meta>`
메타데이터를 정의합니다. 다양한 종류의 `<meta>` 태그가 있습니다.
...
#### `<link>`
외부 리소스를 연결합니다. 보통 CSS 파일을 링크하는 데 사용됩니다.
...
#### `<h1>` to `<h6>`
제목 태그입니다. `<h1>`이 가장 큰 제목, `<h6>`이 가장 작은 제목입니다.
...
<h5>Smaller Subsection Title</h5>
...
```html
<p>This is a paragraph.</p>
<p>This is another paragraph with more content to show line wrapping and spacing.</p>
```
...
```html
<p>This is a line break<br>here.</p>
<p>Example with multiple<br>line breaks<br>to demonstrate.</p>
```
...
#### `<pre>`
미리 서식이 지정된 텍스트를 정의합니다. 공백과 줄 바꿈이 그대로 유지됩니다.

```html
<pre>
  This is preformatted text.
  Notice how       spaces are preserved.
  Line breaks are also kept.
</pre>
```
...
<img src="photo.png" alt="A beautiful scenery" width="500" height="300">
```
...
<div style="background-color: lightgrey; padding: 20px;">
  <p>This div has inline styling.</p>
</div>
```
...
<p>Another example with <span class="highlight">highlighted text</span>.</p>
```

### 스크립트 태그

#### `<noscript>`
스크립트가 비활성화된 경우 대체 콘텐츠를 정의합니다.
...
**Meteor.js**는 풀스택 자바스크립트 프레임워크로, 클라이언트와 서버 사이의 통합을 쉽게 해줍니다. **Blaze**는 Meteor.js와 함께 사용되는 간단하고 반응형 템플릿 엔진입니다. 이 강좌에서는 Meteor.js와 Blaze를 사용하여 간단한 게시글 작성 애플리케이션을 만드는 방법을 단계별로 설명합니다.

### 설치 및 설정

#### Meteor 설치
Meteor를 설치하려면 터미널을 열고 다음 명령어를 실행하세요:
...
#### 새로운 프로젝트 생성
프로젝트 디렉토리를 생성하고 새로운 Meteor 프로젝트를 시작합니다:

```bash
meteor create [프로젝트명] --blaze --release 2.16
cd myApp
```
...
#### Blaze 템플릿 만들기
`client` 폴더에 `main.html` 파일을 생성하고 다음 내용을 추가합니다:
...
#### 템플릿 헬퍼 및 이벤트 추가
`client` 폴더에 `main.js` 파일을 생성하고 다음 내용을 추가합니다:
...
// 방법 1: Posts.find()를 사용하여 포스트를 화면에 가져오기
...
Template.blog.events({
  'submit .new-post'(event) {
    ...
    Posts.insert({ title, content, createdAt: new Date() }); // 방법 1: 직접 insert
    target.title.value = '';
    target.content.value = '';
  },
});

// 방법 2: methods와 ReactiveVar를 사용하여 포스트를 화면에 가져오기
Template.blog.onCreated(function blogOnCreated() {
  this.posts = new ReactiveVar([]);
  Meteor.call('posts.get', (error, result) => {
    if (!error) {
...
Template.blog.events({
  'submit .new-post-method'(event) {
    event.preventDefault();
    const target = event.target;
    const title = target.title.value;
    const content = target.content.value;
    Meteor.call('posts.insert', title, content); // 방법 2: methods 이용한 insert
    target.title.value = '';
    target.content.value = '';
  },
});
...
### MongoDB 사용 및 서버 메소드 추가

#### 콜렉션 생성
`imports/api/posts.js` 파일을 생성하고 다음 내용을 추가합니다:
...
#### 서버 코드 추가
`server/main.js` 파일을 수정하여 다음 내용을 추가합니다:
...
Meteor.startup(() => {
  // 서버 시작 시 초기화 작업을 여기에 추가할 수 있습니다.
});
```
...
#### 사용자 계정 패키지 추가
터미널에서 다음 명령어를 실행하여 사용자 계정 패키지를 추가합니다:
...
#### 로그인 UI 추가
`client/main.html` 파일의 `<body>` 태그에 로그인 UI 템플릿을 추가합니다:
...
#### 보안 설정
`server/main.js` 파일을 수정하여 클라이언트에서 posts 콜렉션에 직접 접근할 수 없도록 설정합니다:
...
Meteor.startup(() => {
  // 클라이언트에서 직접 posts 콜렉션에 접근하지 못하도록 설정
  Posts.deny({
    insert() { return true; },
    update() { return true; },
    remove() { return true; },
